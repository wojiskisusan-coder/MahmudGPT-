...
const LiveMode: React.FC<Props> = ({ isOpen, onClose, voiceId }) => {
    ...
    try {
      await streamChat({
        ...
        onDone: async () => {
          ...
          setIsSpeaking(true);
          try {
            const blob = await convertTextToSpeech(response.slice(0, 2000), selectedVoice);
            await audioController.playBlob(blob, "live-mode");
          } catch {
            speak(response);
          }
          setIsSpeaking(false);
  ...
  return (
    <div className="fixed inset-0 z-50 bg-background/80 backdrop-blur-2xl flex flex-col liquid-glass">
      {/* Header */}
      ...
      <div className="flex-1 flex flex-col items-center justify-center gap-6 p-4 overflow-auto">
        {/* Video feed */}
        <div className={cn(
          "relative w-full max-w-lg rounded-2xl overflow-hidden border border-border/30 bg-black",
          mode === "voice" ? "hidden" : "block"
        )}>
          <video
            ref={videoRef}
